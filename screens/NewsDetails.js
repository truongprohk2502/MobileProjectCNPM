...
import { getSubjectsText, getStudyTypeText, getAddressText, formatMoney, showToastWithGravity } from '../constant/function';
...
function NewsDetails(props) {
    ...
    const requestHandler = async () => {
        setShowAlert(false);
        Axios.post('http://hiringtutors.azurewebsites.net/api/Tutor/OfferToTeach/' + newsData.id, {}, {
            headers: {
                'Authorization': 'Bearer ' + await AsyncStorage.getItem('@token')
            }
        })
            .then(res => {
                showToastWithGravity('Bạn đã đề nghị dạy thành công!');
            })
            .catch(err => {
                console.log(err);
                showToastWithGravity('Đề nghị dạy thất bại!');
            })
        ...
        <View style={{ flex: 1 }}>
            <View style={styles.container}>
                ...
                <View style={styles.pinBtn}>
                    <Text style={{ flex: 1, fontSize: 14, marginTop: 5 }}>Hiện có {newsData.inforNewsView.tutorNewsViews.length} gia sư đã gửi đề nghị dạy</Text>
                    <TouchableHighlight activeOpacity={0.6} underlayColor='#2bbba5' style={styles.btn} onPress={() => setShowAlert(true)}>
            ...
            <AwesomeAlert
                show={showAlert}
                message="Bạn có chắc chắn muốn đề nghị dạy?"
                ...
                onCancelPressed={requestHandler}
